...
// No Redis setup here. Backtesting reaches market-data over HTTP for
// historical bars, the same way trading-engine reaches it for prices --
// neither has a Redis dependency of its own (SPEC.md §2.2).
func TestMain(m *testing.M) {
	if err := setup(); err != nil {
		...
		if errors.Is(err, ErrPostgresUnavailable) {
			skipReason = fmt.Sprintf("backtesting store integration tests unavailable: %v", err)
...
func newStore(t *testing.T) (*store.PostgresBacktestStore, *pgxpool.Pool, context.Context) {
	t.Helper()
	...
	return store.NewPostgresBacktestStore(testPool), testPool, ctx
}
