...
// Package integration holds the trading engine's store-layer tests, which run
// against a real Postgres rather than the in-memory doubles every other suite
...
//
// Nearly all of it is duplicated from services/auth/integration, which is
// where Step 12 built it. That is deliberate. The alternative is extracting it
// into pkg/, which would put a helper that runs DROP DATABASE into the
// dependency graph of every service in the project -- a worse trade than
// duplicating a test file. Revisit if a third service needs it.
//
...
// See SPEC.md §2.4 for the trigger that would change this.
//
// A side effect worth knowing: the test database is migrated from zero every
// time it is created, whereas the dev database was migrated incrementally over
// months. This is the first thing in the repo that proves 001 through 005
// apply cleanly to an empty cluster.
func applyMigrations(ctx context.Context, pool *pgxpool.Pool, root string) error {
	...
	for _, f := range files {
		...
		// filename means reading all five to find the culprit.
...
func truncateAll(tb testing.TB, ctx context.Context, pool *pgxpool.Pool) {
	...
	// CASCADE follows the FK chain from users into accounts, and onward into
	// positions/orders/trades. schema_migrations is untouched.
	if _, err := pool.Exec(ctx, `TRUNCATE TABLE users CASCADE`); err != nil {
		tb.Fatalf("truncating: %v", err)
	}
}

// seedAccount creates a user and their account directly, returning both IDs.
//
// It bypasses the auth service entirely, which is the point: this module does
// not import auth, and an integration suite that needed another service's
// registration path to run would be testing two things at once. Every account
// here starts with exactly the balance the test asks for, rather than auth's
// StartingBalance, so the arithmetic in each assertion stays legible.
func seedAccount(tb testing.TB, ctx context.Context, pool *pgxpool.Pool, balance float64) (userID, accountID uuid.UUID) {
	tb.Helper()

	// Unique per call so several accounts can coexist in one test -- which is
	// what the cross-account isolation and concurrency tests need.
	suffix := uuid.NewString()

	err := pool.QueryRow(ctx,
		`INSERT INTO users (email, username, password_hash) VALUES ($1, $2, $3) RETURNING id`,
		"trader-"+suffix+"@example.test", "trader-"+suffix, "not-a-real-hash").Scan(&userID)
	if err != nil {
	...
	err = pool.QueryRow(ctx,
		...
		userID, balance).Scan(&accountID)
	if err != nil {
		tb.Fatalf("seeding account: %v", err)
	}
	return userID, accountID
}

// seedPosition writes a position row the store did not create, so a sell can
// be tested without first buying through the code under test. Setting up a
// sell with a buy would make the sell tests depend on the buy path being
// correct, which is precisely what they should not assume.
func seedPosition(tb testing.TB, ctx context.Context, pool *pgxpool.Pool, accountID uuid.UUID, symbol string, quantity, avgCost float64) {
	tb.Helper()
	...
	if err != nil {
		tb.Fatalf("seeding position %s: %v", symbol, err)
	}
}
...
// against itself. Step 12 established this for the same reason.
