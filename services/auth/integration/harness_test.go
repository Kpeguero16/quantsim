...
// Package integration holds the auth service's store-layer tests, which run
// against a real Postgres rather than the map-backed mock every other auth
// suite uses.
//
...
// insertUserRaw writes a users row WITHOUT going through the store, which is
// the entire point of it existing.
//
// service.Register lowercases every address before it reaches the store, so
// the store cannot produce a mixed-case stored email. Proving that
// GetUserByEmail matches on lower(email) therefore requires creating a row the
// store itself could never create.
func insertUserRaw(tb testing.TB, ctx context.Context, pool *pgxpool.Pool, email, username, hash string) uuid.UUID {
	tb.Helper()

	var id uuid.UUID
	err := pool.QueryRow(ctx,
		`INSERT INTO users (email, username, password_hash) VALUES ($1, $2, $3) RETURNING id`,
		email, username, hash).Scan(&id)
	if err != nil {
		tb.Fatalf("seeding user %q: %v", email, err)
	}
	return id
}
